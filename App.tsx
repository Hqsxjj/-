...
const App: React.FC = () => {
  ...
  const handleConnectEmby = async () => {
    ...
    try {
      ...
      const response = await fetch(`${cleanUrl}/Library/VirtualFolders?api_key=${embyKey}`);
  ...
  const fetchPosters = async (libraryId: string) => {
    const cleanUrl = embyUrl.replace(/\/$/, '');
    try {
      const resItems = await fetch(`${cleanUrl}/Items?api_key=${embyKey}&ParentId=${libraryId}&Recursive=true&IncludeItemTypes=Movie,Series&Limit=30&SortBy=Random`);
  ...
  const handleApplyToEmby = async () => {
    ...
    try {
      // @ts-ignore
      const canvas = await window.html2canvas(element, { useCORS: true, scale: 2, backgroundColor: null });
      canvas.toBlob(async (blob: Blob) => {
        if (!blob) throw new Error("渲染失败");
        const cleanUrl = embyUrl.replace(/\/$/, '');
        const uploadUrl = `${cleanUrl}/Items/${selectedLibraryId}/Images/Primary?api_key=${embyKey}`;
  ...
  return (
    <div className="min-h-screen flex overflow-hidden font-['Inter',_sans-serif] bg-[#050505] text-zinc-100">
